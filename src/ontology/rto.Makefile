...
## Module for ontology: doid (filter)
## Override: post-process with remove-redundant-subclass.ru to remove
## redundant SubClassOf axioms created by BOT extraction converting
## equivalentClass (intersection) axioms to SubClassOf, combined with
## complement remove stitching children to surviving ancestors.
$(IMPORTDIR)/doid_import.owl: $(MIRRORDIR)/doid.owl $(IMPORTDIR)/doid_terms.txt $(IMPORTSEED) | all_robot_plugins
	$(ROBOT) annotate --input $< --remove-annotations \
		 odk:normalize --add-source true \
		 extract --term-file $(IMPORTDIR)/doid_terms.txt $(T_IMPORTSEED) \
		 ...
		 remove $(foreach p, $(ANNOTATION_PROPERTIES), --term $(p)) \
		        ...
		        --term IAO:0000115 \
		        --term OMO:0002000 \
		        --term-file $(IMPORTDIR)/doid_terms.txt $(T_IMPORTSEED) \
		        --select complement \
